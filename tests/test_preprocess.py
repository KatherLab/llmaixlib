...
def test_preprocess_pdf_with_text(tmp_path, pdf_backend):
    ...
    assert "Re: Medical History and Clinical Course of Patient Ashley Park" in result.replace(os.linesep, " ")
...
def test_preprocess_pdf_with_text_and_ocr(tmp_path, ocr_backend, pdf_backend):
    ...
    if ocr_backend == "ocrmypdf" and pdf_backend == "ocr_backend":
        try:
            preprocess_file(file_path, use_ocr=True, ocr_backend=ocr_backend, pdf_backend=pdf_backend)
        except ValueError as e:
            assert "Cannot use the pdf_backend 'ocr_backend' with ocrmypdf (tesseract)." in str(e)
    else:
        result = preprocess_file(file_path, use_ocr=True, ocr_backend=ocr_backend, pdf_backend=pdf_backend)
...
def test_preprocess_pdf_with_misleading_text_with_ocr(tmp_path, ocr_backend, pdf_backend):
    # Create a temporary file
...
def test_preprocess_pdf_with_misleading_text_force_ocr(tmp_path, ocr_backend, pdf_backend):
    # Create a temporary file
    file_path = Path("tests") / Path("testfiles") / "9874562_misleading_text.pdf"
    ...
    if ocr_backend == "ocrmypdf" and pdf_backend == "ocr_backend":
        ...
        # Call the preprocess_file function
        result = preprocess_file(file_path, use_ocr=True, force_ocr=True, ocr_backend=ocr_backend, pdf_backend=pdf_backend)
